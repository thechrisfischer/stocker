...
class TestAPI(BaseTestConfig):
    ...
    def test_create_new_user(self):
        self.assertIsNone(
            User.query.filter_by(email=self.some_user["email"]).first()
        )
        ...
        self.assertEqual(
            User.query.filter_by(email=self.some_user["email"]).first().email,
    ...
    def test_list_strategies(self):
        res = self.app.get("/api/strategies")
        self.assertEqual(res.status_code, 200)
        data = json.loads(res.data.decode("utf-8"))
        self.assertIn("strategies", data)
        self.assertIn("magic_formula_trailing", data["strategies"])

    def test_rankings_invalid_strategy(self):
        res = self.app.get("/api/rankings/invalid_strategy")
